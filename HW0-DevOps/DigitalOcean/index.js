...
config.token = process.env.NCSU_CSC519_HW0_DigitalOcean_TOKEN;

if( !config.token )
{
	console.log(chalk`{red.bold NCSU_CSC519_HW0_DigitalOcean_TOKEN is not defined!}`);
...
class DigitalOceanProvider
{
	...
	async listImages( )
	{
		...
		let response = await got('https://api.digitalocean.com/v2/images?page=1&per_page=1?type=distribution&per_page=100', { headers: headers, json:true })
							 .catch(err => console.error(`listImaages ${err}`));
...
async function provision()
{
	...
	var image = "centos-6-x32"; // Fill one in from #2
