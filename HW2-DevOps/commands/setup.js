...
async function run(privateKey) {
    ...
    let result = child.spawnSync(`bakerx`, `run ansible-srv bionic --ip 192.168.33.10 --sync`.split(' '), {stdio: 'inherit', shell: true} );
    if( result.error ) { console.log(result); process.exit( result.status ); }

    console.log(chalk.blueBright('Provisioning mattermost server...'));
    result = child.spawnSync(`bakerx`, `run mattermost-srv bionic --ip 192.168.33.80`.split(' '), {stdio: 'inherit', shell: true} );
    if( result.error ) { console.log(result); process.exit( result.status ); }
